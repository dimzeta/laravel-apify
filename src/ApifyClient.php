...
class ApifyClient
{
    ...
    {
        $queryParams = $this->buildSyncQueryParams($options);
    ...
    /**
     * Build query parameters for synchronous run endpoints.
     *
     * @param  array<string, mixed>  $options  Raw options
     * @return array<string, mixed> Filtered and formatted query params
     */
    private function buildSyncQueryParams(array $options): array
    {
        $supported = ['timeout', 'memory', 'build', 'webhooks', 'maxItems', 'maxTotalChargeUsd'];
        $params = [];
